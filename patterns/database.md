...
Behaviours that produce wrong results rather than errors.

**A reused destination struct carries its primary key into the next query.**

```go
var job Job
g.First(&job, 1)   // job.ID is now 1
g.First(&job, 2)   // finds nothing: the destination's key joins the conditions
```

The second call returns `ErrRecordNotFound` even though row 2 exists. Use a
fresh destination per query, or clear the struct. This is the single most
common source of "the row is definitely there and GORM cannot find it".

**`Updates` with a struct skips zero values.** `Updates(Job{Attempts: 0})`
writes nothing, because GORM cannot distinguish "set to zero" from "not
supplied". Use a map when a zero, empty string, or false is a real value:

```go
g.Model(&job).Updates(map[string]any{"attempts": 0, "leased_by": nil})
```

**Every write gets its own transaction unless you turn that off.** GORM wraps
each `Create`/`Update`/`Delete` in an implicit transaction. Set
`SkipDefaultTransaction: true` and open transactions where you mean them —
with `tenancy: shared` the implicit ones do not carry a tenant scope anyway.

**`AutoMigrate` is not a migration tool.** It cannot express policies, partial
indexes, partitions, or column removals, and it versions nothing. Use it for
throwaway test schemas only; real schema changes are SQL in goose.

**`Raw` and `Exec` bypass callbacks and scopes.** Anything relying on a GORM
callback — including SQLite tenant scoping — does not apply to raw statements.
Carry the predicate yourself.
...
Isolating tenants is its own subject, with four ways to build something that
looks like isolation and enforces nothing.

→ **`tenancy: shared`**: [tenancy.md](tenancy.md)
→ **`tenancy: personal`**: nothing to do. Data belongs to one user; use a
  `user_id` foreign key.
